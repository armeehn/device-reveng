# Head unit: root & reverse-engineering

Full EDL backup, TWRP + Magisk root, decompiled vendor apps, and a documented MCU
serial + reverse-camera protocol for the Choiceway / AiNavi **"GT6-EAU"** (Qualcomm
**QCM6125**, Android 13) as fitted to a 2019 Toyota RAV4 (XA50).

> Start at the [README](README.md) for what this repo is and the safety warnings.
> This document is the research half; the launcher lives in [`launcher/`](launcher/).
...
- `rav4-apps/` — **git submodule** → [`armeehn/rav4-apps`](https://github.com/armeehn/rav4-apps): standalone
...
## Getting the vendor files

None of this is redistributed here — it is copyrighted vendor and third-party material.
Source it yourself and drop it in the workspace directory, which defaults to
`~/rav4-headunit` and is overridden with `RAV4_HOME`:

```
$RAV4_HOME/
├── run/
│   ├── prog_firehose_Qcm6125_ddr.elf   # Qualcomm Firehose loader (XDA mega-thread)
│   ├── recovery_ADB.img                # TWRP, USB-device/ADB variant
│   └── Magisk.zip                      # github.com/topjohnwu/Magisk
└── edl/
    ├── src/edl.py                      # github.com/bkerler/edl
    └── venv/bin/python                 # its virtualenv
```

`get-mega-files.sh` fetches the loader and the two TWRP variants from the XDA
mega-thread links. Magisk comes from its own GitHub releases; `edl` is a `git clone`
plus `pip install -r requirements.txt` into `edl/venv`.

Every script checks these paths up front and names the missing one, so a wrong layout
fails immediately instead of halfway through a flash.

**Do not** place firmware, ROM or MCU images inside the repo — `.gitignore` covers the
common names, but the responsibility is yours.
