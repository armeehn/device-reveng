...
#                      └─ setprop riposte.ap.up 1 once wlan1 exists, 0 while it is gone
#
...
# The credentials are published once; whether they are on air is riposte.ap.up, which the app
# waits for before it answers the daemon. At boot the Wi-Fi service often refuses the first
# start-softap, so a raise that fails is retried every RETRY_S, not every CHECK_S.
setprop riposte.ap.ssid "$SSID"
setprop riposte.ap.psk "$PSK"
setprop riposte.ap.iface "$IFACE"
setprop riposte.ap.up 0
